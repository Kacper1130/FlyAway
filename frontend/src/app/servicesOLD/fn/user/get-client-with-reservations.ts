...
export function getClientWithReservations(http: HttpClient, rootUrl: string, params: GetClientWithReservations$Params, context?: HttpContext): Observable<StrictHttpResponse<ClientReservationDto>> {
  const rb = new RequestBuilder(rootUrl, getClientWithReservations.PATH, 'get');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<ClientReservationDto>;
    })
  );
}

getClientWithReservations.PATH = '/api/v1/clients/{id}/reservations';
